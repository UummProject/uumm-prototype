...
This would allow for anyone/anything to be a potential participant (kids, AI, other governances, anonymus entities).

It makes the system more adaptabe (just redefine the merit equation) while adhering to the system goals, and still being compatible with old governance systems (you can always make that a required capacity is to be somone).
...
- The system is backwards compatible (the list of whitelisted identities is a subset of the result of the whitelisted merit)
Smart contracts are extremely well suited for it.
- Allow the system to easly adapt to new circumstnaces
...
If the results of the decisions can be easily measured and the pool of participants is high, [futarchy](https://blog.ethereum.org/2014/08/21/introduction-futarchy/) seems a promising idea but its an apprpoach limited to very specific governances.

These systems are especially interesting because they add a strong incentive layer, that I believe are worth exploring.
